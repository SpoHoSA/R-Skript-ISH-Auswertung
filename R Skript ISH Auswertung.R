...
#Erkl?rung kruscal-Wallis-Test: https://www.youtube.com/watch?v=AESs_KStlYY 
...
#deskriptive ?berpr?fung vor dem eigentlichen Test
library(psych)

describe(data1)

#der Friedman Test = standardprogramm in R --> Kein zus?tzliches Package n?tig
...
#critical.dif = Kritischer Wert = Wert, der vom obs.dif ?berschritten werden muss, damit H0 verworfen wird
friedmanmc(datam)
...
#F?r die Berechnung der Effektst?rke ist der Datensatz im Long Format n?tig
...
#?berraschung
wilcox.test (evalges$X2021_??, evalges$X2022_??, paired = TRUE, correct = TRUE, conf.int = TRUE)

evalges_long <- gather (evalges, t, v, X2021_??:X2022_??)
wilcoxonPairedR (x=evalges_long$v, g = evalges_long$t)
